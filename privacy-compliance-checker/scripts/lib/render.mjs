/**
 * render.mjs — the report renderer, as a function.
 *
 * Split out of the CLI so the same code can run in two places that must never disagree: the
 * skill, which writes a file next to the audit, and Katla's server, which renders an uploaded
 * findings.json for /reports/<uid>. A second implementation would drift, and a report that
 * reads differently depending on where it was produced is worse than no report.
 *
 * Pure: no filesystem, no network, no process exit. Everything it shows comes from `findings`
 * or `brand`, and every value that reaches the output goes through `esc`.
 *
 ...
 * Rejects bad input.
 *
 * Throws rather than exiting: this module runs inside a long-lived server as well as a CLI,
 * and a malformed upload must fail that one request, not take the process down. The CLI
 * catches it and exits, which is the behaviour it always had.
 */
export class ReportInputError extends Error {}
const die = (msg) => {
  throw new ReportInputError(msg);
};
...
/**
 * Renders one audit into a complete HTML report.
 *
 * @param {object} audit     the audit, per references/report-schema.md
 * @param {object} brand     brand tokens, per assets/brand.json — trusted, server-supplied
 * @param {{ artifact?: boolean, logoSvg?: string }} [options]  artifact mode drops the
 *        document shell; logoSvg is the brand wordmark, since this module reads no files
 * @returns {{ html: string, summary: object }}
 */
export function renderReport(audit, brand, options = {}) {
  const data = audit;
  const artifactMode = Boolean(options.artifact);
  const C = brand.color;
  ...
  /* The caller supplies the SVG, because reading it is the one thing this module cannot do
     without touching a filesystem. Absent or unusable, the wordmark falls back to type. */
  const logoMarkup = options.logoSvg
    ? options.logoSvg
        .replace(/<\?xml[^>]*\?>/g, '')
        .replace(/<svg /, `<svg style="height:${brand.logoHeight};width:auto;display:block;" `)
        .trim()
    : `<span style="font-weight:800;font-size:16pt;letter-spacing:-0.03em;color:#fff;">${esc(brand.name)}</span>`;
  ...
  if (cookies.length) {
    // Optional measured-jar columns — rendered only when the audit actually captured them,
    // so reports without jar data keep the original five-column layout.
    const hasLifetime = cookies.some((c) => c.lifetime);
    const hasFlags = cookies.some((c) => c.httpOnly !== undefined || c.secure !== undefined);
    const hasDeclared = cookies.some((c) => c.declared !== undefined);

    const flagCell = (c) => {
      const f = [];
      if (c.httpOnly) f.push('HttpOnly');
      if (c.secure) f.push('Secure');
      if (c.sameSite) f.push(esc(String(c.sameSite)));
      if (!f.length) return `<span style="color:${C.danger};">none</span>`;
      return `<span style="color:${C.caption};">${f.join(' · ')}</span>`;
    };
    const declaredCell = (c) =>
      c.declared === undefined
        ? ''
        : c.declared
          ? `<span style="color:${C.caption};">Yes</span>`
          : `<span style="color:${C.danger};">Not declared</span>`;

    const head = ['Cookie', 'Domain', 'Category'];
    if (hasLifetime) head.push('Lifetime');
    if (hasFlags) head.push('Flags');
    head.push('Purpose', 'Before consent');
    if (hasDeclared) head.push('Declared');
      ...
      appendixBlock(
        ...
        table(
          ...
          cookies.map((c) => {
            ...
            if (hasLifetime) row.push(`<span style="color:${C.caption};">${esc(c.lifetime || '—')}</span>`);
            if (hasFlags) row.push(flagCell(c));
            row.push(esc(c.purpose || ''), preConsentFlag(c));
            if (hasDeclared) row.push(declaredCell(c));
            return row;
          })
        )
      )
  ...
  /* ------------------------------------------------------- artifact variant */
  /* Artifacts are published as page *content*: the host supplies the doctype,
     <head> and <body>, so those tags must not appear in the file. The layout is
     a fixed 210mm sheet, which would overflow a phone, so the artifact build
     also scales each sheet to the viewport. Printing from the published page
     still yields correct A4 — the scaler stands down around print. */

  const ARTIFACT_SCALER = `
  <script>
  (function () {
    var SHEET_PX = 794;           /* 210mm at 96dpi */
    var GUTTER = 16;              /* keep the artifact side-gutter rule */
    function sheets() { return Array.prototype.slice.call(document.querySelectorAll('.sheet')); }
    function apply() {
      var avail = document.documentElement.clientWidth - GUTTER * 2;
      var z = Math.min(1, avail / SHEET_PX);
      sheets().forEach(function (el) { el.style.zoom = z < 1 ? z : ''; });
    }
    function clear() { sheets().forEach(function (el) { el.style.zoom = ''; }); }
    /* Run after the paginator, which measures at natural size. */
    function start() { setTimeout(apply, 0); }
  ...
  let outputHtml = html;
  if (artifactMode) {
    /* In a gallery of these, the domain is what tells one from another — so the
       artifact leads with it rather than with the category word. */
    outputHtml = html
      .replace(
        /<title>[^<]*<\/title>/i,
        `<title>${esc(domain)} · Privacy Audit</title>`
      )
      ...
      .concat(ARTIFACT_SCALER);
  }
  /* The same numbers the CLI prints and Katla files against a lead. Derived here because
     they are already derived here — recomputing them at each call site is how two places
     start disagreeing about whether a site passed. */
  const summary = {
    domain,
    status,
  ...
  return { html: outputHtml, summary };
}
